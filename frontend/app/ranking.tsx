...
export default function Page() {
 ...
 useEffect(()=>{apiFetch('/ranking').then(d=>setItems(d.questoes||d.ranking||[]))},[]);
 return <ScrollView style={s.container} contentContainerStyle={s.content}>
  ...
  <Text style={s.h1}>Ranking</Text>
  {items===null?<ActivityIndicator color={colors.brandPrimary}/>:items.length===0?<Text style={s.empty}>Nenhum item por enquanto.</Text>:items.map((q:any,i:number)=><View key={q.id||q.user_id||i} style={s.card}><Ionicons name="trophy" size={22} color={colors.brandPrimary}/><View style={{flex:1}}><Text style={s.title}>{q.enunciado||`${q.posicao}º  ${q.name}`}</Text><Text style={s.small}>{q.assunto||q.disciplina||`${q.xp} XP • ${q.streak} dias`}</Text></View></View>)}
 </ScrollView>
}
